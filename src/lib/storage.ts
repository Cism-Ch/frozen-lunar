...
if (!process.env.TIGRIS_ACCESS_KEY_ID || !process.env.TIGRIS_SECRET_ACCESS_KEY || !process.env.TIGRIS_BUCKET_NAME) {
...
export const s3Client = new S3Client({
    ...
    credentials: {
        accessKeyId: process.env.TIGRIS_ACCESS_KEY_ID || process.env.AWS_ACCESS_KEY_ID!,
        secretAccessKey: process.env.TIGRIS_SECRET_ACCESS_KEY || process.env.AWS_SECRET_ACCESS_KEY!,
    },
...
export function getPublicUrl(key: string) {
    return `${process.env.NEXT_PUBLIC_TIGRIS_PUBLIC_URL || process.env.TIGRIS_ENDPOINT}/${S3_BUCKET_NAME}/${key}`;
}
