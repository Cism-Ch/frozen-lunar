...
export function handleBetterAuthError(error: unknown): {
    ...
    if (body?.message) {
        ...
        if (
            body.message.includes("Invalid credentials") ||
            body.message.includes("incorrect")
        ) {
            const errorResponse = createErrorResponse(
...
export function handleError(error: unknown): {
    ...
    // Log l'erreur de manière structurée avec Pino
    logger.error("Error handled in application", error);
